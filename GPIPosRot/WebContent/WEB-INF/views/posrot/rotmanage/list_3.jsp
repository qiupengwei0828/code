...
<body>
	...
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/posrot/rotManage/list3">待通知(${status3})</a></li>
	...
	<form:form id="pageForm" modelAttribute="rotation" action="${ctx}/posrot/rotManage/list3" method="post" class="breadcrumb form-search" style="display:none;">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}" />
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}" />
	</form:form>


	<form:form id="sForm" modelAttribute="rotation" action="${ctx}/posrot/rotManage/list3" method="post" class="breadcrumb form-search">
		<label>计划编号：</label>
	...
	<table id="contentTable" class="table table-hover table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="rotation">
			<tr>
				...
				<td><a href="${ctx}/posrot/rotManage/info?id=${rotation.id}&status=${rotation.status}">通知</a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" onclick="downEPlan(${rotation.id})">下载通知书</a></td>
			</tr>
		</c:forEach>
	</table>
	...
	<script type="text/javascript">
		function downEPlan(id) {
			$.ajax({
				...
				success : function(data) {
					window.location.href = "${ctx}/posrot/rotManage/download?fileName=" + data;
				}
			})
		}
	</script>
