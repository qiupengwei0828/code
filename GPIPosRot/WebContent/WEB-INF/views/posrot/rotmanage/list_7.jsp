...
<body>
	...
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/posrot/rotManage/list7">待归档(${status7})</a></li>
	</ul>

	<form:form id="pageForm" modelAttribute="rotation" action="${ctx}/posrot/rotManage/list7" method="post" class="breadcrumb form-search" style="display:none;">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}" />
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}" />
	</form:form>


	<form:form id="sForm" modelAttribute="rotation" action="${ctx}/posrot/rotManage/list7" method="post" class="breadcrumb form-search">
		<label>计划编号：</label>
		...
		<input id="btnSubmit" class="btn btn-primary" type="button" value="计划归档" />
	...
	<table id="contentTable" class="table table-hover table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="rotation">
			<tr>
				...
				<td><a href="${ctx}/posrot/rotManage/info?id=${rotation.id}">归档</a></td>
			</tr>
		</c:forEach>
	</table>
