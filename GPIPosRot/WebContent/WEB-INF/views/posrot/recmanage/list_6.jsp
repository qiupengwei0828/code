...
<body>
	...
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/posrot/recManage/list_items?status=6">待执行(${status6})</a></li>
		<li><a href="${ctx}/posrot/recManage/list_items?status=7">待归档(${status7})</a></li>
	</ul>



	<form:form id="pageForm" modelAttribute="recess" action="${ctx}/posrot/recManage/list_items?status=6" method="post" class="breadcrumb form-search" style="display:none;">
	...
	<form:form id="sForm" modelAttribute="recess" action="${ctx}/posrot/recManage/list_items?status=6" method="post" class="breadcrumb form-search">
		...
		<input id="btnSubmit" class="btn btn-primary" type="button" value="确认执行" />
	</form:form>
	...
	<table id="contentTable" class="table table-hover table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="recess">
			<tr>
				...
				<td><a href="${ctx}/posrot/recManage/info?id=${recess.id}">确认执行</a></td>
			</tr>
		</c:forEach>
	</table>
	...
	<div id="psaa_title" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		...
		<div class="modal-body">
			<h5>确认强修计划已执行？</h5>
			<input type="hidden" id="opeOpinion" name="opeOpinion" />
		</div>
		<div class="modal-footer">
			<button class="btn btn-primary" id="yesPass">已执行</button>
