...
public class ExportRecessService {

	// /导出
	public void export(List<Recess> list, HttpServletResponse response) throws Exception {
		response.reset();
		response.addHeader("Content-Type", "applicationnd.ms-excel; charset=UTF-8");
		String filename = "中国邮政储蓄银行甘肃省分行从业人员强制休假花名册";
	...
	private void exportExcel(List<Recess> list, OutputStream out) throws Exception {
		// 声明一个EXCEL工作薄
		HSSFWorkbook myWorkbook = new HSSFWorkbook();
		// 生成一个sheet
		HSSFSheet mySheet = myWorkbook.createSheet("从业人员强制休假花名册");
		// 设置默认值
	...
	private void CreateTable(HSSFSheet sheet, HSSFWorkbook workbook, List<Recess> list) {
		...
		setValueByRowAndCol(0, 0, "所在单位", sheet, cellStyle, format);
		setValueByRowAndCol(0, 1, "姓名", sheet, cellStyle, format);
		setValueByRowAndCol(0, 2, "岗位", sheet, cellStyle, format);
		setValueByRowAndCol(0, 3, "姓名", sheet, cellStyle, format);
		setValueByRowAndCol(0, 4, "岗位", sheet, cellStyle, format);
		setValueByRowAndCol(0, 5, "离岗日期", sheet, cellStyle, format);
		setValueByRowAndCol(0, 6, "到岗日期", sheet, cellStyle, format);
		setValueByRowAndCol(0, 7, "休假天数", sheet, cellStyle, format);
		setValueByRowAndCol(0, 8, "备注", sheet, cellStyle, format);

		for (int i = 0; i < list.size(); i++) {
			Recess recess = list.get(i);
			setValueByRowAndCol(i + 1, 0, recess.getOrgName2nd(), sheet, cellStyle1, format);
			setValueByRowAndCol(i + 1, 1, recess.getRecUserName(), sheet, cellStyle1, format);
			setValueByRowAndCol(i + 1, 2, recess.getRecOrgName(), sheet, cellStyle1, format);
			setValueByRowAndCol(i + 1, 3, recess.getRepUserName(), sheet, cellStyle1, format);
			setValueByRowAndCol(i + 1, 4, recess.getRepOrgName(), sheet, cellStyle1, format);
			setValueByRowAndCol(i + 1, 5, recess.getRecBeginDate(), sheet, cellStyle1, format);
			setValueByRowAndCol(i + 1, 6, recess.getHanDate(), sheet, cellStyle1, format);
			setValueByRowAndCol(i + 1, 7, recess.getVacationDay(), sheet, cellStyle1, format);
			setValueByRowAndCol(i + 1, 8, recess.getRemark(), sheet, cellStyle1, format);
