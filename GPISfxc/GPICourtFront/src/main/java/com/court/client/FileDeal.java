...
import com.court.tools.core.util.FileOperate;
import com.court.tools.core.util.PropertiesUtil;
import com.court.tools.core.util.uuid.IdUtil;
...
//    private static String  bakQueryContentPath=PropertiesUtil.readValue("sfxc.properties", "court.query.file.bak.path");//协查请求报文已处理目录
//    private static String bakMsgFilePath=PropertiesUtil.readValue("sfxc.properties", "court.msg.file.path");//返回消息--已处理报文
	...
	public static void saveQueryContent(String queryContent){
		String  queryContentPath = PropertiesUtil.readValue("sfxc.properties", "court.query.file.path");//协查请求报文存储目录
 ...
 * @return
 */
	public static String getBackcontent(String method){
		String rtnFilePath= PropertiesUtil.readValue("sfxc.properties", "court.return.file.path");//反馈报文路径
		String bakRtnFilePath=PropertiesUtil.readValue("sfxc.properties", "court.return.file.bak.path");//反馈报文已处理路径
    	...
    	if(path.exists())
    	{
	    	...
	    	for(int i=0;i<t.length;i++)
	    	{
	    		...
	    		try {
			         ...
			         //将已处理的反馈文件 移动到 已处理目录
			         logger.info("反馈报文反馈后移动至已反馈目录处理：，移动："+t[i].getName()+" 到"+bakRtnFilePath);
	...
	public static void  saveMsg(String xmlMsg){
		String msgFilePath=PropertiesUtil.readValue("sfxc.properties", "court.msg.file.path");//返回消息--待处理报文
		String fileName=IdUtil.getId();
	...
	public static void savaQueryFile(String bdhm,String wjmc,byte[] wjnr){
		String wsinfoPath=PropertiesUtil.readValue("sfxc.properties", "court.query.wsinfo.path");//协查请求--协查文书
