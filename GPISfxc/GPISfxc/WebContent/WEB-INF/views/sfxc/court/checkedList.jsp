...
<body>
 <form:form id="searchForm" modelAttribute="courtQueryInfo" action="${ctx}/court/check/checkedList" method="post" class="breadcrumb form-search" style="display:none;">
	...
	<form:form id="searchForm" modelAttribute="courtQueryInfo" action="${ctx}/court/check/checkedList" method="post" class="breadcrumb form-search">
	    <label>承办检察官：</label>
		...
		<c:forEach items="${page.list}" var="courtQueryInfo">
			<tr>
			    ...
			    <td>${courtQueryInfo.endDt}</td>
			     <td><a
					href="${ctx}/court/check/checked?queryId=${courtQueryInfo.queryId}">查看详情</a></td>		   
			</tr>
		</c:forEach>
	</table>
