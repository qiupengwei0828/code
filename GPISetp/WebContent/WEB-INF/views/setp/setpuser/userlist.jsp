	...
	$(document).ready(function() {
		$("#export").click(function() {
			...
			if (beginDate != "" && endDate != "") {
				window.location.href = "${ctx}/setp/export/sum_user_setp?beginDate=" + beginDate + "&endDate=" + endDate + "&orgNo=" + orgNo;
			} else {
	...
	<script type="text/javascript">
		...
		function pageInit() {
			var gridData = $("#jqGrid").jqGrid({
				url : "${ctx}/setp/setpUser/get_sum_user_list?orgNo=" + orgNo,
				datatype : "json",
				regional : 'cn',//本地化
				colNames : [ '排名', '员工姓名', '地市', '部门', '手机号', '合计步数' ],
				colModel : [ {
					name : 'rownum',//数据列名
				//width : 70
				}, {
		...
		$("#submitBtn").click(function() {
			...
			$("#jqGrid").jqGrid('setGridParam', {
				url : "${ctx}/setp/setpUser/get_sum_user_list",
				datatype : 'json',
				//type : "POST",
				//需要传的参数
				postData : {
