package com.setp.service.exportusersum;
...
public class ExportUserSum {

	// /导出
	public void export(List<SetpUser> list, HttpServletResponse response) throws Exception {
		response.reset();
		response.addHeader("Content-Type", "applicationnd.ms-excel; charset=UTF-8");
		String filename = "个人计步统计";
	...
	private void exportExcel(List<SetpUser> list, OutputStream out) throws Exception {
		// 声明一个EXCEL工作薄
		HSSFWorkbook myWorkbook = new HSSFWorkbook();
		// 生成一个sheet
		HSSFSheet mySheet = myWorkbook.createSheet("个人计步统计明细");
		// 设置默认值
	...
	private void CreateTable(HSSFSheet sheet, HSSFWorkbook workbook, List<SetpUser> list) {
		...
		setValueByRowAndCol(0, 1, "员工姓名", sheet, cellStyle, format);
		setValueByRowAndCol(0, 2, "机构", sheet, cellStyle, format);
		setValueByRowAndCol(0, 3, "手机号", sheet, cellStyle, format);
		setValueByRowAndCol(0, 4, "和计步数", sheet, cellStyle, format);

		for (int i = 0; i < list.size(); i++) {
			SetpUser setpUser = list.get(i);
			setValueByRowAndCol(i + 1, 0, setpUser.getRownum(), sheet, cellStyle1, format);
			setValueByRowAndCol(i + 1, 1, setpUser.getUserName(), sheet, cellStyle1, format);
			setValueByRowAndCol(i + 1, 2, setpUser.getOrgName(), sheet, cellStyle1, format);
			setValueByRowAndCol(i + 1, 3, setpUser.getMobile(), sheet, cellStyle1, format);
			setValueByRowAndCol(i + 1, 4, setpUser.getSetpNum(), sheet, cellStyle1, format);
		}
