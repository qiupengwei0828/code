...
(function(window, undefined) {
	...
	jQuery.extend({
		...
		access : function(elems, fn, key, value, chainable, emptyGet, pass) {
			...
			if (key && typeof key === "object") {
				...
				if (bulk) {
					// Bulk operations only iterate when executing function
					// values
					if (exec) {
						exec = fn;
	...
	jQuery.event = {
		...
		dispatch : function(event) {
			...
			var i, j, cur, ret, selMatch, matched, matches, handleObj, sel, related, handlers = ((jQuery._data(this, "events") || {})[event.type] || []), delegateCount = handlers.delegateCount, args = core_slice.call(arguments), run_all = !event.exclusive && !event.namespace, special = jQuery.event.special[event.type]
					|| {}, handlerQueue = [];
			...
			filter : {
				...
				"ATTR" : function(name, operator, check) {
					return function(elem, context) {
						...
						return operator === "=" ? result === check : operator === "!=" ? result !== check : operator === "^=" ? check && result.indexOf(check) === 0 : operator === "*=" ? check && result.indexOf(check) > -1 : operator === "$=" ? check
								&& result.substr(result.length - check.length) === check : operator === "~=" ? (" " + result + " ").indexOf(check) > -1 : operator === "|=" ? result === check || result.substr(0, check.length + 1) === check + "-" : false;
					};
				},
			...
			for (i = 0; (elem = elems[i]) != null; i++) {
				...
				if (typeof elem === "string") {
					if (!rhtml.test(elem)) {
						...
						// Take out of fragment container (we need a fresh div
						// each time)
