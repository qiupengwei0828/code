...
<link href="${ctx}/js/autocomplete/jquery.autocomplete.css" rel="stylesheet" />
  ...
  $(document).ready(function() {
     $("#useDate").val($("#useDateDate").val());
     $("#date").val($("#dateDate").val());
  });
/*
   * 分页
	 */
	...
	function exportData(){
		    var url="${ctx}/bd/use/exportData?certactName="+$("#certactName").val()+"&certactCode="
		    +$("#certactCode").val()+"&orgNo="+$("#orgNo").val()+"&useType="+$("#useType").val()+"&useDate="+$("#useDate").val()+"&date="+$("#date").val();
		    window.location.href=url;
		}
  </script>
<body>

	<form:form id="searchForm" modelAttribute="bdUseDetail" action="${ctx}/bd/use/listReport" method="post" class="breadcrumb form-search" style="display:none;">
		...
		<input type="hidden" id="orgNo" name="orgNo" value="${bdUseDetail.orgNo}">
	</form:form>
	
	<!--  
	<bd:message content="${message}"/>-->
	<form:form id="searchForm" modelAttribute="bdUseDetail" action="${ctx}/bd/use/listReport" method="post" class="breadcrumb form-search">
	   <label>单证代码：</label>
		...
		<label>使用类型：</label>
		<form:select path="useType" class="input-medium" id="useType">
		...
		<input id="down" style="float:right" class="btn btn-primary" type="button" value="导出数据" onclick="exportData()" /> 
	</form:form>
