...
<body>
	<ul class="nav nav-tabs">
		...
		<form:form id="searchForm" modelAttribute="store" action="${ctx}/bd/bdplan/index" method="post" class="breadcrumb form-search" style="display:none;">
	...
	<form:form id="searchForm" modelAttribute="bdPlan" action="${ctx}/bd/bdplan/index" method="post" class="breadcrumb form-search">		
		<label>分行名称：</label>
		<sys:treeselect id="orgNo" name="orgNo" value="${store.orgNo}" labelName="orgName"  labelValue="${store.orgName}" title="分行" url="/bd/bdstore/treeData" cssStyle="width:150px;font-size:10px;" />
		<label>请领机构：</label>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="bdPlan">
			<tr>
				...
				<c:if test="${bdPlan.status!=05}">
				<a href="${ctx}/bd/bdplan/updateform?id=${bdPlan.id}">编辑</a>
