...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/bd/bdplan/audit">请领审批列表</a></li>
	</ul>
		<form:form id="searchForm" modelAttribute="store" action="${ctx}/bd/bdplan/audit" method="post" class="breadcrumb form-search" style="display:none;">
	...
	<form:form id="searchForm" modelAttribute="bdPlan" action="${ctx}/bd/bdplan/audit" method="post" class="breadcrumb form-search">		
	    <label>分行名称：</label>
		...
		<c:forEach items="${page.list}" var="bdPlan">
			<tr>
                ...
                <c:if test="${bdPlan.status==02}">
					<a href="${ctx}/bd/bdplan/check?id=${bdPlan.id}">审核</a>
			    </c:if>
			    <c:if test="${bdPlan.status!=02}">
			    <c:if test="${bdPlan.status!=03}">
			    <a href="${ctx}/bd/bdplan/delete?id=${bdPlan.id}" onclick="return confirmx('确认要删除该请领信息吗？', this.href)">删除</a>
			    </c:if>
			    </c:if>
			    <c:if test="${bdPlan.status==03}">
			    <a href="${ctx}/bd/bdplan/check?id=${bdPlan.id}">详情</a>
			    </c:if>
